...
export default function MaterialsList({ classroomId, userRole, userId }) {
  ...
  const fetchMaterials = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://sece-space.onrender.com'}/api/classrooms/${classroomId}/resources`);
      if (response.ok) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://sece-space.onrender.com'}/api/upload`, {
        method: 'POST',
        body: formData,
      });
